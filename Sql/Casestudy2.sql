...
select distinct(DIM_LOCATION.state) from FACT_TRANSACTIONS left join DIM_LOCATION on DIM_LOCATION.IDLocation=FACT_TRANSACTIONS.IDLocation where year(fact_transactions.date)>'2004'

--2.What state in the US is buying more 'Samsung' cell phones? 

select top 1 DIM_LOCATION.State from FACT_TRANSACTIONS left join DIM_MODEL on DIM_MODEL.IDModel=FACT_TRANSACTIONS.IDModel left join DIM_MANUFACTURER on DIM_MODEL.IDManufacturer=DIM_MANUFACTURER.IDManufacturer left join DIM_LOCATION on DIM_LOCATION.IDLocation=FACT_TRANSACTIONS.IDLocation where DIM_LOCATION.Country='US' and DIM_MANUFACTURER.Manufacturer_Name='samsung' group by DIM_LOCATION.State order by count(FACT_TRANSACTIONS.IDCustomer) desc

--3. Show the number of transactions for each model per zip code per state.

select DIM_LOCATION.ZipCode,DIM_LOCATION.State, DIM_MODEL.IDModel,DIM_MODEL.Model_Name,count(*)[Number of Transactions] from FACT_TRANSACTIONS left join DIM_MODEL on DIM_MODEL.IDModel=FACT_TRANSACTIONS.IDModel left join DIM_LOCATION on DIM_LOCATION.IDLocation=FACT_TRANSACTIONS.IDLocation group by DIM_LOCATION.ZipCode,DIM_LOCATION.State, DIM_MODEL.IDModel,DIM_MODEL.Model_Name

--4. Show the cheapest cellphone  

select top 1 DIM_MODEL.IDModel,DIM_MODEL.Model_Name,DIM_MANUFACTURER.Manufacturer_Name from DIM_MODEL left join DIM_MANUFACTURER on DIM_MODEL.IDManufacturer=DIM_MANUFACTURER.IDManufacturer order by DIM_MODEL.Unit_price asc

--5. Find out the average price for each model in the top5 manufacturers in 
--terms of sales quantity and order by average price.

select DIM_MODEL.IDModel,DIM_MODEL.Model_Name,DIM_MANUFACTURER.Manufacturer_Name,AVG(FACT_TRANSACTIONS.TotalPrice)[Average] from DIM_MANUFACTURER left join DIM_MODEL on DIM_MANUFACTURER.IDManufacturer=DIM_MODEL.IDManufacturer left join FACT_TRANSACTIONS on FACT_TRANSACTIONS.IDModel=DIM_MODEL.IDModel where DIM_MANUFACTURER.Manufacturer_Name in (select top 5 DIM_MANUFACTURER.Manufacturer_Name from DIM_MANUFACTURER left join DIM_MODEL on DIM_MANUFACTURER.IDManufacturer=DIM_MODEL.IDManufacturer left join FACT_TRANSACTIONS on FACT_TRANSACTIONS.IDModel=DIM_MODEL.IDModel group by DIM_MANUFACTURER.Manufacturer_Name order by sum(FACT_TRANSACTIONS.Quantity)desc) group by DIM_MODEL.IDModel,DIM_MODEL.Model_Name,DIM_MANUFACTURER.Manufacturer_Name order by Average desc


--6. List the names of the customers and the average amount spent in 2009, 
--where the average is higher than 500

select DIM_CUSTOMER.Customer_Name,avg(FACT_TRANSACTIONS.TotalPrice)[Average] from DIM_CUSTOMER left join FACT_TRANSACTIONS on DIM_CUSTOMER.IDCustomer=FACT_TRANSACTIONS.IDCustomer where year(FACT_TRANSACTIONS.Date)='2009' group by DIM_CUSTOMER.Customer_Name having avg(FACT_TRANSACTIONS.TotalPrice)>500 order by avg(FACT_TRANSACTIONS.TotalPrice) desc

--7. List if there is any model that was in the top 5 in terms of quantity, 
--simultaneously in 2008, 2009 and 2010
 
select * from (select top 5 FACT_TRANSACTIONS.IDModel from FACT_TRANSACTIONS where year(FACT_TRANSACTIONS.Date)='2008' group by FACT_TRANSACTIONS.IDModel order by sum(FACT_TRANSACTIONS.Quantity) desc)as a
intersect
select * from(select top 5 FACT_TRANSACTIONS.IDModel from FACT_TRANSACTIONS where year(FACT_TRANSACTIONS.Date)='2009' group by FACT_TRANSACTIONS.IDModel order by sum(FACT_TRANSACTIONS.Quantity) desc)as b
intersect
select * from (select top 5 FACT_TRANSACTIONS.IDModel from FACT_TRANSACTIONS where year(FACT_TRANSACTIONS.Date)='2010' group by FACT_TRANSACTIONS.IDModel order by sum(FACT_TRANSACTIONS.Quantity) desc)as c

--8. Show the manufacturer with the 2nd top sales in the year of 2009 and the 
--manufacturer with the 2nd top sales in the year of 2010. 

select * from(select top 1 * from (select top 2 DIM_MANUFACTURER.IDManufacturer,DIM_MANUFACTURER.Manufacturer_Name,year(FACT_TRANSACTIONS.Date)[Year],sum(FACT_TRANSACTIONS.TotalPrice)[Sales] from DIM_MANUFACTURER left join DIM_MODEL on DIM_MANUFACTURER.IDManufacturer=DIM_MODEL.IDManufacturer left join FACT_TRANSACTIONS on DIM_MODEL.IDModel=FACT_TRANSACTIONS.IDModel where year(FACT_TRANSACTIONS.Date)='2009' group by DIM_MANUFACTURER.IDManufacturer,DIM_MANUFACTURER.Manufacturer_Name,year(FACT_TRANSACTIONS.Date) order by sales desc)
as a order by sales) as ab
union all
select * from(select top 1 * from (select top 2 DIM_MANUFACTURER.IDManufacturer,DIM_MANUFACTURER.Manufacturer_Name,year(FACT_TRANSACTIONS.Date)[Year],sum(FACT_TRANSACTIONS.TotalPrice)[Sales] from DIM_MANUFACTURER left join DIM_MODEL on DIM_MANUFACTURER.IDManufacturer=DIM_MODEL.IDManufacturer left join FACT_TRANSACTIONS on DIM_MODEL.IDModel=FACT_TRANSACTIONS.IDModel where year(FACT_TRANSACTIONS.Date)='2010' group by DIM_MANUFACTURER.IDManufacturer,DIM_MANUFACTURER.Manufacturer_Name,year(FACT_TRANSACTIONS.Date) order by sales desc)
as b order by sales asc)as bc

--9. Show the manufacturers that sold cellphone in 2010 but didn’t in 2009.

select DIM_MANUFACTURER.IDManufacturer,DIM_MANUFACTURER.Manufacturer_Name from DIM_MANUFACTURER left join DIM_MODEL on DIM_MANUFACTURER.IDManufacturer=DIM_MODEL.IDManufacturer left join FACT_TRANSACTIONS on DIM_MODEL.IDModel=FACT_TRANSACTIONS.IDModel where year(FACT_TRANSACTIONS.Date)='2010' group by  DIM_MANUFACTURER.IDManufacturer,DIM_MANUFACTURER.Manufacturer_Name
except
select DIM_MANUFACTURER.IDManufacturer,DIM_MANUFACTURER.Manufacturer_Name from DIM_MANUFACTURER left join DIM_MODEL on DIM_MANUFACTURER.IDManufacturer=DIM_MODEL.IDManufacturer left join FACT_TRANSACTIONS on DIM_MODEL.IDModel=FACT_TRANSACTIONS.IDModel where year(FACT_TRANSACTIONS.Date)='2009' group by  DIM_MANUFACTURER.IDManufacturer,DIM_MANUFACTURER.Manufacturer_Name
...
select top 100 DIM_CUSTOMER.IDCustomer,DIM_CUSTOMER.Customer_Name,DIM_CUSTOMER.Email,DIM_CUSTOMER.Phone,year(FACT_TRANSACTIONS.Date),avg(FACT_TRANSACTIONS.TotalPrice)[Average Spend],avg(FACT_TRANSACTIONS.Quantity)[Average Quantity]from DIM_CUSTOMER left join FACT_TRANSACTIONS on DIM_CUSTOMER.IDCustomer=FACT_TRANSACTIONS.IDCustomer group by DIM_CUSTOMER.IDCustomer,DIM_CUSTOMER.Customer_Name,DIM_CUSTOMER.Email,DIM_CUSTOMER.Phone,year(FACT_TRANSACTIONS.Date) order by [Average Spend] desc
